...
export const storageService = {
  ...
  login: async (usernameOrEmail: string, password: string): Promise<{ success: boolean; user?: UserProfile; error?: string }> => {
    ...
    try {
      ...
      if (authError?.message?.toLowerCase().includes('email not confirmed')) {
        ...
        try { await supabase.rpc('admin_confirm_user_email', { p_email: email }); } catch { /* ignoré */ }
        // Retenter
  ...
  register: async (
    ...
    try {
      ...
      try { await supabase.rpc('admin_confirm_user_email', { p_email: finalEmail }); } catch { /* ignoré */ }

/**
 * LiveTeacher.tsx — v4 STABLE
 * ────────────────────────────────────────────────────────────────
 * Corrections vs version précédente :
 * ✅ Crédits débités UNE SEULE FOIS au démarrage (pas à chaque reconnexion)
 * ✅ ai.live.connect() → Promise<session> correctement awaitée (@google/genai 0.2.x)
 * ✅ isConnectedRef gate : audio envoyé seulement après onopen
 * ✅ Cleanup AudioContext fiable (pas de fuite mémoire)
 * ✅ Reconnexion auto 3× avec backoff exponentiel 1.5s/3s/6s
 */
...
// ── Utilitaires audio ─────────────────────────────────────────────────────────
const b64ToPcmBuffer = (base64: string, ctx: AudioContext): AudioBuffer => {
...
const downsample = (buf: Float32Array, fromRate: number, toRate: number): Float32Array => {
  ...
  for (let i = 0; i < result.length; i++) {
    ...
    let sum = 0, count = 0;
    for (let j = s; j < e && j < buf.length; j++) { sum += buf[j]; count++; }
    result[i] = count > 0 ? sum / count : 0;
  }
  return result;
};

const f32ToPcm16B64 = (f32: Float32Array): string => {
  ...
  for (let i = 0; i < bytes.length; i += 8192) {
    out += String.fromCharCode(...Array.from(bytes.subarray(i, Math.min(i + 8192, bytes.length))));
  }
  return btoa(out);
};

const WORKLET_CODE = `
class TmAudioCapture extends AudioWorkletProcessor {
  constructor() { super(); this._buf = new Float32Array(2048); this._w = 0; }
  process(inputs) {
    const ch = inputs[0]?.[0];
    if (!ch) return true;
    for (let i = 0; i < ch.length; i++) {
      this._buf[this._w++] = ch[i];
      if (this._w >= this._buf.length) { this.port.postMessage(this._buf.slice()); this._w = 0; }
    }
    return true;
  }
}
registerProcessor('tm-capture', TmAudioCapture);
`;
...
const LiveTeacher: React.FC<LiveTeacherProps> = ({
  ...
  const playChunk = useCallback(async (base64: string) => {
    ...
    try {
      ...
      const buf    = b64ToPcmBuffer(base64, ctx);
    ...
    } catch (e) { console.warn('[LiveTeacher] playChunk error:', e); }
  }, []);
  ...
  const startMicrophone = useCallback(async (ctx: AudioContext) => {
    ...
    const node = new AudioWorkletNode(ctx, 'tm-capture');
    processorRef.current = node;

    node.port.onmessage = (e) => {
      // ✅ Gate : audio envoyé seulement quand session ouverte (onopen déclenché)
      ...
      const b64 = f32ToPcm16B64(ds);
      try {
        sessionRef.current.sendRealtimeInput({
          media: { mimeType: `audio/pcm;rate=${INPUT_SAMPLE_RATE}`, data: b64 }
        });
      } catch (err) { console.warn('[LiveTeacher] sendRealtimeInput:', err); }
    };
  ...
  const startSession = useCallback(async () => {
    ...
    // ✅ Débit crédits UNE SEULE FOIS (pas à chaque reconnexion automatique)
    ...
    try {
      // ✅ import.meta.env (Vite) — supporte plusieurs clés séparées par virgule
      // @ts-ignore
...
Utilise le Français uniquement pour de brèves explications si l'élève est bloqué.
Protocole de correction : encourage → donne la bonne forme → demande de répéter.
Commence par te présenter brièvement en ${user.preferences?.targetLanguage || 'anglais'} et demander comment va l'élève.`;

      const ai = new GoogleGenAI({ apiKey });

      // ✅ @google/genai 0.2.x : live.connect() retourne une Promise<LiveSession>
      const sessionPromise = ai.live.connect({
        ...
        callbacks: {
          onopen: () => {
            ...
            console.log('[LiveTeacher] ✅ Session ouverte');
          },
          onmessage: async (msg: any) => {
            ...
            if (msg.serverContent?.turnComplete) {
              ...
              if (c && nextStartRef.current < c.currentTime) nextStartRef.current = c.currentTime;
            }
          },
          onclose: (e: any) => {
            console.log('[LiveTeacher] Session fermée:', e?.code, e?.reason);
            if (!isMountedRef.current) return;
            isConnectedRef.current = false;
            sessionRef.current     = null;
            ...
            if (count < MAX_RECONNECTS) {
              ...
              const delay = Math.pow(2, count) * 1500; // 1.5s → 3s → 6s
      ...
      // ✅ Attendre la résolution de la Promise → session object disponible
    ...
    <div className="fixed inset-0 z-[150] bg-[#050505] flex flex-col font-sans overflow-hidden">
      ...
      <div className="flex-1 flex flex-col items-center justify-center relative w-full mb-10">
        ...
        <div className={`relative z-20 w-48 h-48 rounded-full bg-[#0F1422] flex items-center justify-center transition-all duration-500 shadow-2xl ${
          ...
          <div className={`absolute bottom-3 right-3 w-5 h-5 rounded-full border-2 border-[#0F1422] ${status === 'connected' ? 'bg-emerald-400 animate-pulse' : 'bg-slate-600'}`} />
        </div>
        ...
        <div className="flex items-center gap-6">
          <button onClick={() => setIsMuted(m => !m)} disabled={status !== 'connected'}
            className={`w-16 h-16 rounded-full flex items-center justify-center transition-all shadow-lg disabled:opacity-40 ${
              isMuted ? 'bg-red-500/20 border-2 border-red-500/50 text-red-400 hover:bg-red-500/30'
          ...
          <button disabled className="w-16 h-16 rounded-full bg-slate-800 border-2 border-slate-700 text-slate-500 flex items-center justify-center opacity-40 cursor-default">
